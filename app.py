...
db_config = {
    'host': os.getenv("MYSQL_HOST", "monorail.proxy.rlwy.net"),
    'port': int(os.getenv("MYSQL_PORT", 50332)),
    'user': os.getenv("MYSQL_USER", "root"),
    'password': os.getenv("MYSQL_PASSWORD", "your_password"),
    'database': os.getenv("MYSQL_DATABASE", "therapy_management"),
}
        ...
        if user and check_password_hash(user['Password'], password):
            ...
            if user['Role'] == 'Patient':
                ...
                if patient:
                    ...
                    flash("Patient name not found.", "danger")
                ...
                if therapist:
                    ...
                    flash("Therapist name not found.", "danger")
...
def register():
    if request.method == 'POST':
        ...
        if not re.match(r"[^@]+@[^@]+\.[^@]+", contact_info):
        ...
        try:
            if role == 'admin':
                ...
                if cursor.fetchone():
                    ...
                    # Send confirmation email
                    subject = "Registration Successful"
                    body = f"Dear {name},\n\nYour Therapist account has been successfully registered.\n\nThank you for joining us!"
                    send_email(contact_info, subject, body)
...
def send_email(receiver_email, subject, body):
    ...
    password = "dfnf vjfh lgvw rcsq"  # Replace with your app password
...
def appointment_history():
    if 'role' in session and session['role'] == 'Patient':
        ...
        # Fetch all past appointments and cancelled appointments for the logged-in patient
        cursor.execute(''' 
            SELECT 
            ...
            WHERE A.PatientID = %s AND (CONCAT(A.Date, ' ', A.Time) < %s OR A.Status = 'Cancelled')
            ORDER BY A.Date DESC, A.Time DESC
        ''', (session['user_id'], now))  # Use session to get the patient’s ID
        
        appointments = cursor.fetchall()

        # Add logic to check if feedback can be left and update status if needed
        for appointment in appointments:
            ...
            appointment['can_leave_feedback'] = now >= full_appointment_time + timedelta(hours=1)
...
def therapist_past_appointments():
    if 'role' in session and session['role'] == 'Therapist':
        ...
        try:
            ...
            cursor.execute('''
                SELECT A.AppointmentID, P.Name, P.Age, P.TherapyGoals, P.Diagnosis, A.Date, A.Time, A.Status, F.Comments,PR.Amount,PR.PaymentMethod
                FROM Appointments AS A
                JOIN Patients AS P ON A.PatientID = P.PatientID
                LEFT JOIN PaymentRecords PR ON A.AppointmentID = PR.AppointmentID
                LEFT JOIN Feedback AS F ON A.AppointmentID = F.AppointmentID
                WHERE A.TherapistID = %s AND A.Date < CURDATE()
                ORDER BY A.Date DESC, A.Time DESC
            ''', (therapist_id,))
            ...
            for appointment in past_appointments:
                ...
                # Optional: handle 'Cancelled' status if any logic needs to be added
                # If the status is 'Cancelled' and the appointment time has passed, leave it as 'Cancelled'
                elif appointment['Status'] == 'Cancelled' and now >= appointment_time:
                    # Optional: additional logic can be added here if required
                    pass
...
def verify_therapist(therapist_id):
    try:
        ...
        cursor.execute("UPDATE Therapists SET IsVerified = TRUE WHERE TherapistID = %s", (therapist_id,))
        conn.commit()
        ...
        if therapist and therapist[0]:
            ...
            password = "lwut bhpb suvw ltbr"  # Replace with your app password
        ...
        return jsonify({"error": "Failed to verify therapist"}), 500
...
def delete_user(user_id):
    try:
        ...
        cursor.execute("SELECT Role, PatientID, TherapistID FROM users WHERE UserID = %s", (user_id,))
        user = cursor.fetchone()
...
def delete_appointment(appointment_id):
    ...
    try:
        ...
        therapist_body = f"""
        <p>Dear {appointment['TherapistName']},</p>
